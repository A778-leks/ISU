...
let Native = function (l) {
	...
	let i = l.generics;
	let g = l.initialize;
	let h = l.afterImplement || function () {};
	let d = g || j;
	i = i !== false;
...
function $pick() {
	for (let b = 0, a = arguments.length; b < a; b++) {
...
Array.implement({
	...
	indexOf: function (c, d) {
		let a = this.length;
		for (let b = (d < 0) ? Math.max(0, a + d) : d || 0; b < a; b++) {
			if (this[b] === c) {
				return b;
			}
		}
	...
	link: function (c) {
		let a = {};
		for (let f = 0, b = this.length; f < b; f++) {
			for (let d in c) {
				if (c[d](this[f])) {
					a[d] = this[f];
					delete c[d];
...
String.implement({
	...
	hexToRgb: function (b) {
		let a = this.match(/^#?(\w{1,2})(\w{1,2})(\w{1,2})$/);
		return (a) ? a.slice(1).hexToRgb(b) : null;
	},
	rgbToHex: function (b) {
		let a = this.match(/\d{1,3}/g);
		return (a) ? a.rgbToHex(b) : null;
	},
	stripScripts: function (b) {
		let a = "";
		let c = this.replace(/<script[^>]*>([\s\S]*?)<\/script>/gi, function () {
			a += arguments[1] + "\n";
			return "";
		});
		if (b === true) {
...
function Class(b) {
	...
	let a = function () {
		...
		this._current = $empty;
		let c = (this.initialize) ? this.initialize.apply(this, arguments) : this;
...
Browser.detect = function () {
	for (let b in this.Engines) {
		let a = this.Engines[b]();
		if (a) {
			this.Engine = {
				name: b,
				version: a
...
Browser.Plugins.Flash = (function () {
	let a = ($try(function () {
		...
		return new ActiveXObject("ShockwaveFlash.ShockwaveFlash").Getletiable("$version");
	}) || "0 r0").match(/\d+/g);
	return {
...
Native.UID = 1;
let $uid = (Browser.Engine.trident) ? function (a) {
	return (a.uid || (a.uid = [Native.UID++]))[0];
} : function (a) {
	return a.uid || (a.uid = Native.UID++);
};
let Window = new Native({
	name: "Window",
...
let Element = new Native({
	name: "Element",
	legacy: window.Element,
	initialize: function (a, b) {
		let c = Element.Constructors.get(a);
		if (c) {
			return c(b);
		}
		if (typeof a == "string") {
	...
	afterImplement: function (a, b) {
		...
		Elements.implement(a, function () {
			let c = [],
				h = true;
			for (let f = 0, d = this.length; f < d; f++) {
				let g = this[f][a].apply(this[f], arguments);
				c.push(g);
				if (h) {
					h = ($type(g) == "element");
				}
...
let IFrame = new Native({
	name: "IFrame",
	generics: false,
	initialize: function () {
		let g = Array.link(arguments, {
			properties: Object.type,
			iframe: $defined
		});
		let d = g.properties || {};
		let c = document.id(g.iframe);
		let f = d.onload || $empty;
		delete d.onload;
		...
		let b = function () {
			...
			if (!h || h == window.location.host) {
				let i = new Window(c.contentWindow);
...
(function () {
	let d;
	try {
		let a = document.createElement("<input name=x>");
		d = (a.name == "x");
	} catch (b) {}
	let c = function (f) {
...
Native.implement([Element, Document], {
	...
	getElements: function (a, d) {
		...
		let b = (a.length > 1);
		a.each(function (f) {
			let g = this.getElementsByTagName(f.trim());
			(b) ? c.extend(g): c = g;
		}, this);
...
(function () {
	...
	let b = ["compact", "nowrap", "ismap", "declare", "noshade", "checked", "disabled", "readonly", "multiple", "selected", "noresize", "defer"];
	let l = ["value", "type", "defaultValue", "accessKey", "cellPadding", "cellSpacing", "colSpan", "frameBorder", "maxLength", "readOnly", "rowSpan", "tabIndex", "useMap"];
	b = b.associate(b);
	Hash.extend(f, b);
	...
	Element.implement({
		...
		getProperty: function (n) {
			let m = f[n];
			let o = (m) ? this[m] : this.getAttribute(n, 2);
		...
		getElementById: function (p, o) {
			let n = this.ownerDocument.getElementById(p);
			if (!n) {
				return null;
			}
			for (let m = n.parentNode; m != this; m = m.parentNode) {
				if (!m) {
					return null;
				}
			}
		...
		getComputedStyle: function (n) {
			...
			let m = this.getDocument().defaultView.getComputedStyle(this, null);
		...
		toQueryString: function () {
			...
			this.getElements("input, select, textarea", true).each(function (n) {
				...
				let o = (n.tagName.toLowerCase() == "select") ? Element.getSelected(n).map(function (p) {
					return p.value;
			...
			let o = function (w, v) {
				...
				if (Browser.Engine.trident) {
					...
					if (w.options) {
						let x = w.options,
							t = v.options;
						for (let u = x.length; u--;) {
							x[u].selected = t[u].selected;
						}
					}
				}
				let y = j[v.tagName.toLowerCase()];
				if (y && v[y]) {
					w[y] = v[y];
				}
			};
			if (p) {
				let q = s.getElementsByTagName("*"),
...
Element.Properties.html = (function () {
	...
	let b = {
		set: function () {
			let f = Array.flatten(arguments).join("");
			let g = Browser.Engine.trident && a[this.get("tag")];
			if (g) {
				let h = c;
...
if (Browser.Engine.webkit && Browser.Engine.version < 420) {
	Element.Properties.text = {
		get: function () {
			...
			let a = this.ownerDocument.newElement("div", {
				html: this.innerHTML
	...
	Element.implement({
		...
		getOffsets: function () {
			if (this.getBoundingClientRect) {
				let k = this.getBoundingClientRect(),
		...
		getPosition: function (l) {
			...
			let m = this.getOffsets(),
				j = this.getScrolls();
			let i = {
				x: m.x - j.x,
				y: m.y - j.y
			};
			let k = (l && (l = document.id(l))) ? l.getPosition() : {
				x: 0,
				y: 0
			};
			return {
	...
	let d = Element.getComputedStyle;
...
let Event = new Native({
	...
	initialize: function (a, g) {
		...
		this.$extended = true;
		let k = a.type;
		let h = a.target || a.srcElement;
		while (h && h.nodeType == 3) {
			h = h.parentNode;
		}
		if (k.test(/key/)) {
			let b = a.which || a.keyCode;
			let n = Event.Keys.keyOf(b);
			if (k == "keydown") {
				let d = b - 111;
				if (d > 0 && d < 13) {
					n = "f" + d;
			...
			if (k.match(/(click|mouse|menu)/i)) {
				...
				if (k.match(/DOMMouseScroll|mousewheel/)) {
					let i = (a.wheelDelta) ? a.wheelDelta / 120 : -(a.detail || 0) / 3;
				}
				let f = (a.which == 3) || (a.button == 2);
				let m = null;
				if (k.match(/over|out/)) {
					switch (k) {
						case "mouseover":
...
Native.implement([Element, Window, Document], {
	...
	removeEvent: function (c, b) {
		let a = this.retrieve("events");
		if (!a || !a[c]) {
			return this;
		}
		let g = a[c].keys.indexOf(b);
		if (g == -1) {
			return this;
		}
		a[c].keys.splice(g, 1);
		let f = a[c].values.splice(g, 1)[0];
		let d = Element.Events.get(c);
		if (d) {
			if (d.onRemove) {
...
Element.implement({
	...
	setStyle: function (b, a) {
		...
		b = b.camelCase();
		if ($type(a) != "string") {
			let c = (Element.Styles.get(b) || "@").split(" ");
	...
	getStyle: function (h) {
		...
		if (!$chk(a)) {
			a = [];
			for (let g in Element.ShortStyles) {
				if (h != g) {
					continue;
				}
				for (let f in Element.ShortStyles[g]) {
		...
		if (Browser.Engine.presto || (Browser.Engine.trident && !$chk(parseInt(a, 10)))) {
			if (h.test(/^(height|width)$/)) {
				let b = (h == "width") ? ["left", "right"] : ["top", "bottom"],
					d = 0;
				b.each(function (i) {
...
["Top", "Right", "Bottom", "Left"].each(function (h) {
	let g = Element.ShortStyles;
	let b = Element.Styles;
...
let Fx = new Class({
	...
	step: function () {
		...
		if (a < this.time + this.options.duration) {
			let b = this.transition((a - this.time) / this.options.duration);
...
Fx.CSS = new Class({
	...
	search: function (a) {
		...
		Array.each(document.styleSheets, function (f, d) {
			...
			let g = f.rules || f.cssRules;
			Array.each(g, function (k, h) {
				if (!k.style) {
					return;
				}
				let j = (k.selectorText) ? k.selectorText.replace(/^\w+/, function (i) {
...
Fx.Morph = new Class({
	...
	start: function (b) {
		...
		let f = {},
			d = {};
		for (let c in b) {
			let a = this.prepare(this.element, c, b[c]);
			f[c] = a.from;
			d[c] = a.to;
		}
...
Fx.implement({
	getTransition: function () {
		let a = this.options.transition || Fx.Transitions.Sine.easeInOut;
		if (typeof a == "string") {
			let b = a.split(":");
			a = Fx.Transitions;
...
Fx.Tween = new Class({
	...
	start: function (c, f, d) {
		...
		let b = Array.flatten(arguments);
		this.property = this.options.property || b.shift();
		let a = this.prepare(this.element, this.property, b);
		return this.parent(a.from, a.to);
	}
});
Element.Properties.tween = {
	set: function (a) {
		let b = this.retrieve("tween");
		if (b) {
			b.cancel();
		}
	...
	fade: function (c) {
		...
		switch (c) {
			...
			case "hide":
				...
				break;
			case "toggle":
				let b = this.retrieve("fade:flag", this.get("opacity") == 1);
				f.start(d, (b) ? 0 : 1);
...
let Request = new Class({
	...
	send: function (l) {
		...
		if (this.options.format) {
			let k = "format=" + this.options.format;
			h = (h) ? k + "&" + h : k;
		}
		...
		if (this.options.urlEncoded && a == "post") {
			let c = (this.options.encoding) ? "; charset=" + this.options.encoding : "";
			this.headers.set("Content-type", "application/x-www-form-urlencoded" + c);
		}
		if (this.options.noCache) {
			let g = "noCache=" + new Date().getTime();
			h = (h) ? g + "&" + h : g;
		}
		let f = b.lastIndexOf("/");
...
Request.HTML = new Class({
	...
	processHTML: function (c) {
		...
		return $try(function () {
			...
			for (let g = 0, f = d.childNodes.length; g < f; g++) {
				let j = Element.clone(d.childNodes[g], true, true);
				if (j) {
					a.grab(j);
				}
			}
...
let Cookie = new Class({
	...
	read: function () {
		let a = this.options.document.cookie.match("(?:^|;)\\s*" + this.key.escapeRegExp() + "=([^;]*)");
...
Element.implement({
	match: function (b) {
		...
		let d = Selectors.Utils.parseTagAndID(b);
		let a = d[0],
			f = d[1];
		if (!Selectors.Filters.byID(this, f) || !Selectors.Filters.byTag(this, a)) {
			return false;
		}
		let c = Selectors.Utils.parseSelector(b);
...
Selectors.Utils = {
	...
	parseNthArgument: function (i) {
		...
		let f = i.match(/^([+-]?\d*)?([a-z]+)?([+-]?\d*)?$/);
		if (!f) {
			return false;
		}
		let h = parseInt(f[1], 10);
		let d = (h || h === 0) ? h : 1;
		let g = f[2] || false;
		let c = parseInt(f[3], 10) || 0;
		if (d != 0) {
			c--;
			while (c < 1) {
				c += d;
	...
	parseTagAndID: function (b) {
		let a = b.match(Selectors.RegExps.tag);
		let c = b.match(Selectors.RegExps.id);
	...
	getByTagAndID: function (b, a, d) {
		if (d) {
			let c = (b.getElementById) ? b.getElementById(d, true) : Element.getElementById(b, d, true);
	...
	search: function (p, o, u) {
		let b = [];
		let c = o.trim().replace(Selectors.RegExps.splitter, function (k, j, i) {
			b.push(j);
			return ":)" + i;
		}).split(":)");
		let q, f, B;
		for (let A = 0, w = c.length; A < w; A++) {
			...
			let a = b[A - 1];
			let r = Selectors.Utils.parseTagAndID(z);
			let C = r[0],
				s = r[1];
			if (A == 0) {
				...
				let d = {},
					h = [];
				for (let y = 0, x = q.length; y < x; y++) {
					h = Selectors.Getters[a](h, q[y], C, s, d);
				}
				q = h;
			}
			let g = Selectors.Utils.parseSelector(z);
			if (g) {
				f = [];
				for (let v = 0, t = q.length; v < t; v++) {
					B = q[v];
...
Selectors.Getters = {
	" ": function (j, h, k, a, f) {
		let d = Selectors.Utils.getByTagAndID(h, k, a);
	...
	">": function (j, h, k, a, g) {
		let c = Selectors.Utils.getByTagAndID(h, k, a);
...
Selectors.Filters = {
	...
	byAttribute: function (c, d, b, f) {
		let a = Element.prototype.getProperty.call(c, d);
		if (!a) {
			return (b == "!=");
		}
		if (!b || f == undefined) {
...
Selectors.Pseudo = new Hash({
	...
	"nth-child": function (h, f) {
		h = (h == undefined) ? "n" : h;
		let c = Selectors.Utils.parseNthArgument(h);
		if (c.special != "n") {
...
let Swiff = new Class({
	...
	initialize: function (m, n) {
		...
		this.setOptions(n);
		n = this.options;
		let b = this.id = n.id || this.instance;
		let a = document.id(n.container);
		...
		f.flashlets = Hash.toQueryString(h);
		if (Browser.Engine.trident) {
...
Swiff.remote = function (obj, fn) {
	let rs = obj.CallFunction('<invoke name="' + fn + '" returntype="javascript">' + __flash__argumentsToXML(arguments, 2) + "</invoke>");
	return eval(rs);
};